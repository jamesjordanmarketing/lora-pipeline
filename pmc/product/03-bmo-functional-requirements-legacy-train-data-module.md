...
- Shared components in `src/components/lora-pipeline/`
...
- `POST /api/lora-pipeline/generate/single` - Generate one conversation
- `POST /api/lora-pipeline/generate/batch` - Generate multiple conversations
- `POST /api/lora-pipeline/generate/process-all` - Generate all remaining
- `GET /api/lora-pipeline/conversations` - Fetch conversations with filters
- `GET /api/lora-pipeline/conversations/[id]` - Fetch conversation details
- `POST /api/lora-pipeline/import/seed` - Import seed JSON data
- `GET /api/lora-pipeline/quality/metrics` - Fetch quality dashboard data
- `PATCH /api/lora-pipeline/conversations/[id]/status` - Update status
- `POST /api/lora-pipeline/conversations/[id]/approve` - Approve conversation
...
1. **Database Schema Specification:**
   - Location: `C:\Users\james\Master\BrightHub\brun\lora-pipeline\pmc\product\03-bmo-functional-requirements-lora-pipeline-module.md`
   - Section: "SQL Migration Script"
   - This contains the complete SQL to create all tables

2. **Seed Conversation JSON Files:**
   - `C:\Users\james\Master\BrightHub\brun\lora-pipeline\pmc\context-ai\pmct\training-data-seeds\c-alpha-build_v3.4-LoRA-FP-convo-10-first_v1.json` (conversations 1-9, 38 turns)
   - `C:\Users\james\Master\BrightHub\brun\lora-pipeline\pmc\context-ai\pmct\training-data-seeds\c-alpha-build_v3.4-LoRA-FP-convo-10-complete.json` (conversation 10, 3 turns)
